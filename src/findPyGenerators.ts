...
export function findPyGenerators () {
    ...
    $("div[data-pygen]").each(function (j, eg) { 
        ...
        const checkBtn = $('<button class="btn btn-sm btn-primary"><i class="fas fa-check"></i> Comprova</button>') as JQuery<HTMLButtonElement>;
        const comodiBtn = $('<button class="btn btn-sm btn-warning" style="display:none;"><i class="far fa-life-ring"></i> Comodí 50%</button>') as JQuery<HTMLButtonElement>;
        ...
        const createDynamicMathquill = function() {
         ...
         const showmeBtn = $('<button class="btn btn-sm btn-outline-info" title="Mostra la resposta"><i class="fas fa-question-circle"></i> Solució</button>') as JQuery<HTMLButtonElement>;
